...
function ve(e) {
	...
	return n === 0 ? t.map((o) => ({
		...
		let l = 0;
		if (s.collapsedSize !== void 0) l = T(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.collapsedSize
		}) / n * 100);
		let r;
		if (s.defaultSize !== void 0) r = T(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.defaultSize
		}) / n * 100);
		let a = 0;
		if (s.minSize !== void 0) a = T(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.minSize
		}) / n * 100);
		let c = 100;
		if (s.maxSize !== void 0) c = T(ie({
			groupSize: n,
			panelElement: i,
			styleProp: s.maxSize
		}) / n * 100);
...
function Je(e) {
	const { element: t, orientation: n, panels: o, separators: i } = e, s = be(n, Array.from(t.children).filter(qe).map((x) => ({ element: x }))).map(({ element: x }) => x), l = [];
	let r = !1, a = !1, c = -1, f = -1, g = 0, d, b = [];
	{
		let x = -1;
		for (const u of s) u.hasAttribute("data-panel") && (x++, u.hasAttribute("data-disabled") || (g++, c === -1 && (c = x), f = x));
	}
	if (g > 1) {
		let x = -1;
		for (const u of s) if (u.hasAttribute("data-panel")) {
			x++;
			const p = o.find((m) => m.element === u);
			if (p) {
				if (d) {
					...
					} else b.length ? v = b : v = [n === "horizontal" ? new DOMRect(m.right, S.top, S.left - m.right, S.height) : new DOMRect(S.left, m.bottom, S.width, S.top - m.bottom)];
					for (const y of v) {
						...
						if (h.width < w) {
							const P = w - h.width;
							h = new DOMRect(h.x - P / 2, h.y, h.width + P, h.height);
						}
						if (h.height < w) {
							const P = w - h.height;
							h = new DOMRect(h.x, h.y - P / 2, h.width, h.height + P);
						}
						!r && !(x <= c || x > f) && l.push({
							group: e,
...
function le({ delta: e, initialLayout: t, panelConstraints: n, pivotIndices: o, prevLayout: i, trigger: s }) {
	...
	switch (s) {
		case "keyboard":
			{
				...
				C(p, `Panel constraints not found for index ${u}`);
				const { collapsedSize: m = 0, collapsible: S, minSize: v = 0 } = p;
				if (S) {
					const y = r[u];
					if (C(y != null, `Previous layout not found for panel index ${u}`), D(y, m)) {
						const h = v - y;
			...
			}
			{
				...
				C(p, `No panel constraints found for index ${u}`);
				const { collapsedSize: m = 0, collapsible: S, minSize: v = 0 } = p;
				if (S) {
					const y = r[u];
					if (C(y != null, `Previous layout not found for panel index ${u}`), D(y, v)) {
						const h = y - m;
						A(h, Math.abs(e)) > 0 && (e = e < 0 ? 0 - h : h);
					}
				}
		...
		default: {
			...
			if (S && A(m, y) < 0) if (e > 0) {
				const h = y - v, w = h / 2;
				A(m + e, y) < 0 && (e = A(e, w) <= 0 ? 0 : h);
			} else {
				const h = y - v, w = 100 - h / 2;
				A(m - e, y) < 0 && (e = A(100 + e, w) > 0 ? 0 : -h);
			}
			break;
		}
	}
	{
		...
		for (;;) {
			...
			if (m += h, p += u, p < 0 || p >= n.length) break;
		}
	...
	}
	{
		...
		for (; p >= 0 && p < n.length;) {
			...
			C(S != null, `Previous layout not found for panel index ${p}`);
			const v = S - m, y = Z({
				overrideDisabledPanels: l,
				panelConstraints: n[p],
				prevSize: S,
			...
			if (!D(S, y) && (d += S - y, c[p] = y, d.toFixed(3).localeCompare(Math.abs(e).toFixed(3), void 0, { numeric: !0 }) >= 0)) break;
			e < 0 ? p-- : p++;
		}
	}
	if (Ot(a, c)) return i;
	{
		...
		C(p != null, `Previous layout not found for panel index ${u}`);
		const m = p + d, S = Z({
			overrideDisabledPanels: l,
		...
		if (c[u] = S, !D(S, m)) {
			...
			for (; h >= 0 && h < n.length;) {
				const w = c[h];
				C(w != null, `Previous layout not found for panel index ${h}`);
				const M = w + v, P = Z({
					overrideDisabledPanels: l,
				...
				if (D(w, P) || (v -= P - w, c[h] = P), D(v, 0)) break;
				e > 0 ? h-- : h++;
			}
		}
	}
	if (!D(Object.values(c).reduce((u, p) => p + u, 0), 100, .1)) return i;
...
function U({ layout: e, panelConstraints: t }) {
	...
	if (!D(i, 100) && o.length > 0) for (let r = 0; r < t.length; r++) {
		const a = o[r];
	...
	if (!D(s, 0)) for (let r = 0; r < t.length; r++) {
		const a = o[r];
		C(a != null, `No layout data found for index ${r}`);
		const c = a + s, f = Z({
			overrideDisabledPanels: !0,
		...
		if (a !== f && (s -= f - a, o[r] = f, D(s, 0))) break;
	}
...
function tt({ groupId: e, panelId: t }) {
	const n = () => {
		const r = X();
		for (const [a, { defaultLayoutDeferred: c, derivedPanelConstraints: f, layout: g, groupSize: d, separatorToPanels: b }] of r) if (a.id === e) return {
		...
		const a = s();
		if (r === a) return;
		const { defaultLayoutDeferred: c, derivedPanelConstraints: f, group: g, groupSize: d, layout: b, separatorToPanels: x } = n(), u = g.panels.findIndex((v) => v.id === t), p = u === g.panels.length - 1, S = U({
			layout: le({
				delta: p ? a - r : r - a,
				initialLayout: b,
				panelConstraints: f,
	...
	return {
		...
		getSize: () => {
			const { group: r } = n(), a = s(), { element: c } = i();
			return {
				asPercentage: a,
		...
		resize: (r) => {
			const { group: a } = n(), { element: c } = i(), f = ne({ group: a });
			l(T(ie({
				groupSize: f,
				panelElement: c,
				styleProp: r
...
function nt({ groupId: e }) {
	...
	return {
		...
		setLayout(n) {
			const { defaultLayoutDeferred: o, derivedPanelConstraints: i, group: s, groupSize: l, layout: r, separatorToPanels: a } = t(), c = U({
				layout: n,
				panelConstraints: i
			});
			return o ? r : (W(r, c) || $(s, {
...
function B(e, t) {
	const n = pe(e), o = H(n.id, !0), i = n.separators.find((g) => g.element === e);
	C(i, "Matching separator not found");
	const s = o.separatorToPanels.get(i);
	C(s, "Matching panels not found");
	const l = s.map((g) => n.panels.indexOf(g)), a = nt({ groupId: n.id }).getLayout(), f = U({
		layout: le({
			delta: t,
			initialLayout: a,
...
function Ge(e) {
	...
	if (!n.disabled) switch (e.key) {
		...
		case "Enter": {
			e.preventDefault();
			const o = pe(t), { derivedPanelConstraints: s, layout: l, separatorToPanels: r } = H(o.id, !0), a = o.separators.find((d) => d.element === t);
			C(a, "Matching separator not found");
			const c = r.get(a);
			C(c, "Matching panels not found");
			const f = c[0], g = s.find((d) => d.panelId === f.id);
			if (C(g, "Panel metadata not found"), g.collapsible) {
				const d = l[f.id];
				B(t, (g.collapsedSize === d ? o.mutableState.expandedPanelSizes[f.id] ?? g.minSize : g.collapsedSize) - d);
			}
			break;
		}
		case "F6": {
			e.preventDefault();
			const i = pe(t).separators.map((a) => a.element), s = Array.from(i).findIndex((a) => a === e.currentTarget);
...
function at({ document: e, event: t, hitRegions: n, initialLayoutMap: o, mountedGroups: i, pointerDownAtPoint: s, prevCursorFlags: l }) {
	let r = 0;
	n.forEach((c) => {
		...
		if (!p || !m) return;
		const { defaultLayoutDeferred: S, derivedPanelConstraints: v, groupSize: y, layout: h, separatorToPanels: w } = m;
		if (v && h && w) {
			const M = le({
				delta: u,
				initialLayout: p,
...
function jt({ group: e, nextGroupSize: t, prevGroupSize: n, prevLayout: o }) {
	...
	if (r.forEach((g, d) => {
		f[d] = g;
	}), s > 0) for (const g of a) f[g] = T((o[g] ?? 0) / s * c);
	else {
		const g = T(c / a.length);
...
function Vt(e) {
		...
		for (const p of u) {
			...
			if (S === e.element) {
				if (t) {
					...
					const y = H(e.id);
					if (!y) return;
					const h = ve(e), w = y.defaultLayoutDeferred ? We(h) : y.layout, P = U({
						layout: jt({
							group: e,
							nextGroupSize: v,
					...
					if (!y.defaultLayoutDeferred && W(y.layout, P) && $t(y.derivedPanelConstraints, h) && y.groupSize === v) return;
					$(e, {
	...
	s.observe(e.element), e.panels.forEach((u) => {
		C(!o.has(u.id), `Panel ids must be unique; id "${u.id}" was used more than once`), o.add(u.id), u.onResize && s.observe(u.element);
	});
	const l = ne({ group: e }), r = ve(e), a = e.panels.map(({ id: u }) => u).join(",");
	let c = e.mutableState.defaultLayout;
	c && (Ht(e.panels, c) || (c = void 0));
	const g = U({
		layout: e.mutableState.layouts[a] ?? c ?? We(r),
		panelConstraints: r
	...
	return Je(e).forEach((u) => {
		...
		C(!i.has(u.id), `Separator ids must be unique; id "${u.id}" was used more than once`), i.add(u.id), u.element.addEventListener("keydown", Ge);
		...
		}), J.get(d) || (d.removeEventListener("dblclick", Oe, !0), d.removeEventListener("pointerdown", Ae, !0), d.removeEventListener("pointerleave", je), d.removeEventListener("pointermove", He), d.removeEventListener("pointerout", Ve), d.removeEventListener("pointerup", Be, !0)), s.disconnect();
	};
}
function Bt() {
...
function Ut({ children: e, className: t, defaultLayout: n, disableCursor: o, disabled: i, elementRef: s, groupRef: l, id: r, onLayoutChange: a, onLayoutChanged: c, orientation: f = "horizontal", resizeTargetMinimumSize: g = {
	coarse: 20,
	fine: 10
}, style: d, ...b }) {
	...
	const M = se((z, L) => {
		...
		if (E) {
			...
			switch (k.state) {
				case "active":
					I = k.hitRegions.some((V) => V.group === R);
					break;
			}
			return {
		...
		registerPanel: (z) => {
			const L = h.current;
			return L.panels = be(f, [...L.panels, z]), y(), () => {
				L.panels = L.panels.filter((k) => k !== z), y();
			};
		},
		registerSeparator: (z) => {
			const L = h.current;
			return L.separators = be(f, [...L.separators, z]), y(), () => {
				L.separators = L.separators.filter((k) => k !== z), y();
			};
		},
		updatePanelProps: (z, { disabled: L }) => {
			const R = h.current.panels.find((V) => V.id === z);
		...
		updateSeparatorProps: (z, { disabled: L, disableDoubleClick: k }) => {
			const E = h.current.separators.find((I) => I.id === z);
		...
		const oe = ze(m, (j) => {
			...
			R.mutableState.layouts[ut] = ce, Ee.forEach((_) => {
				if (_.collapsible) {
					...
					if (ge) {
						const ft = D(_.collapsedSize, ce[_.panelId]), dt = D(_.collapsedSize, ge[_.panelId]);
	...
	if (i) try {
		...
		if (t) {
			...
			if (r && Array.isArray(r.layout) && t.length === r.layout.length) {
				const a = {};
				for (let c = 0; c < t.length; c++) a[t[c]] = r.layout[c];
				return a;
			}
		} else {
			...
			if (l.length === 1) {
				...
				if (r && Array.isArray(r.layout)) {
					...
					if (a.length === r.layout.length) {
						const c = {};
						for (let f = 0; f < a.length; f++) c[a[f]] = r.layout[f];
						return c;
					}
				}
			}
...
function on({ debounceSaveMs: e = 100, panelIds: t, storage: n = localStorage, ...o }) {
	const i = t !== void 0, s = "id" in o ? o.id : o.groupId, l = he(s, t ?? []), r = (0, import_react.useSyncExternalStore)(Xt, () => n.getItem(l), () => n.getItem(l)), a = (0, import_react.useMemo)(() => {
		if (r) {
			const u = JSON.parse(r), p = Object.values(u);
			if (Array.from(p).every((m) => typeof m == "number")) return u;
		}
		...
		t,
		n
	]), f = a ?? c, g = (0, import_react.useRef)(null), d = (0, import_react.useCallback)(() => {
		const u = g.current;
	...
	const b = (0, import_react.useCallback)((u) => {
		d();
		let p;
	...
	return {
		...
		*/
		onLayoutChange: (0, import_react.useCallback)((u) => {
...
function Yt({ children: e, className: t, collapsedSize: n = "0%", collapsible: o = !1, defaultSize: i, disabled: s, elementRef: l, groupResizeBehavior: r = "preserve-relative-size", id: a, maxSize: c = "100%", minSize: f = "0%", onResize: g, panelRef: d, style: b, ...x }) {
	const u = !!a, p = Le(a), m = Re({ disabled: s }), S = (0, import_react.useRef)(null), v = Ce(S, l), { getPanelStyles: y, id: h, orientation: w, registerPanel: M, updatePanelProps: P } = Me(), G = g !== null, N = se((R, E, I) => {
		g?.(R, a, I);
	});
	q(() => {
		const R = S.current;
	...
	}, L = (0, import_react.useSyncExternalStore)((R) => ze(h, R), z, z);
	let k;
...
function Qt({ children: e, className: t, disabled: n, disableDoubleClick: o, elementRef: i, id: s, style: l, ...r }) {
	const a = Le(s), c = Re({
		disabled: n,
		disableDoubleClick: o
	}), [f, g] = (0, import_react.useState)({}), [d, b] = (0, import_react.useState)("inactive"), [x, u] = (0, import_react.useState)(!1), p = (0, import_react.useRef)(null), m = Ce(p, i), { disableCursor: S, id: v, orientation: y, registerSeparator: h, updateSeparatorProps: w } = Me(), M = y === "horizontal" ? "vertical" : "horizontal";
	q(() => {
		const N = p.current;
		if (N !== null) {
			const z = {
				...
				b(E.next.state !== "inactive" && E.next.hitRegions.some((I) => I.separator === z) ? E.next.state : "inactive");
			}), R = ze(v, (E) => {
