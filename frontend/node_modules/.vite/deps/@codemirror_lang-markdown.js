import { A as syntaxTree, E as languageDataProp, L as EditorView, Nt as EditorSelection, O as sublanguageProp, Pt as EditorState, Qt as countColumn, Rt as Prec, S as indentNodeProp, T as indentUnit, a as LanguageDescription, cn as NodeSet, dn as Parser, f as defineLanguageFacet, fn as Tree, g as foldInside, i as Language, in as tags$1, l as bracketMatchingHandle, ln as NodeType, m as flatIndent, nn as Tag, o as LanguageSupport, on as IterMode, p as delimitedIndent, pn as parseMixed, r as LRLanguage, rn as styleTags, s as ParseContext, sn as NodeProp, u as continuedIndent, un as NodeWeakMap, ut as keymap, v as foldNodeProp, y as foldService } from "./dist-dvrLPuvE.js";
import { i as LocalTokenGroup, n as ExternalTokenizer, r as LRParser, t as ContextTracker } from "./dist-BwcOcrr3.js";
import { a as completeFromList, c as snippetCompletion, s as ifNotIn, t as CompletionContext } from "./dist-J70ZTjUZ.js";
...
function isOrderedList(line, cx, breaking) {
	...
	if (pos == line.pos || pos > line.pos + 9 || next != 46 && next != 41 || pos < line.text.length - 1 && !space$2(line.text.charCodeAt(pos + 1)) || breaking && !inList(cx, Type.OrderedList) && (line.skipSpace(pos + 1) == line.text.length || pos > line.pos + 1 || line.next != 49)) return -1;
...
var DefaultBlockParsers = {
	...
	ATXHeading(cx, line) {
		...
		if (after == endOfSpace || after == off || !space$2(line.text.charCodeAt(after - 1))) after = line.text.length;
...
*/
var parser$3 = new MarkdownParser(new NodeSet(nodeTypes).extend(markdownHighlighting), Object.keys(DefaultBlockParsers).map((n) => DefaultBlockParsers[n]), Object.keys(DefaultBlockParsers).map((n) => DefaultLeafBlocks[n]), Object.keys(DefaultBlockParsers), DefaultEndLeaf, DefaultSkipMarkup, Object.keys(DefaultInline).map((n) => DefaultInline[n]), Object.keys(DefaultInline), []);
...
var colon = 58, parenL = 40, underscore = 95, bracketL$1 = 91, dash = 45, period = 46, hash = 35, percent = 37, ampersand = 38, backslash = 92, newline$1 = 10, asterisk = 42;
function isAlpha(ch) {
...
var descendant = new ExternalTokenizer((input) => {
	if (space$1.includes(input.peek(-1))) {
		let { next } = input;
		if (isAlpha(next) || next == underscore || next == hash || next == period || next == asterisk || next == bracketL$1 || next == colon && isAlpha(input.peek(1)) || next == dash || next == ampersand) input.acceptToken(descendantOp);
	}
});
...
var commonmark = /* @__PURE__ */ parser$3.configure({ props: [
