...
var _MessageContentParser = (_class = class {
  
  __init() {this._lastIndex = 0}
  ...
  parse(content) {
    ...
    const attributes = match[1].split(/\s+/).slice(1).filter((x) => x !== WRAP_OPEN_TAG_2).map((x) => x.split(":")).reduce(
      (acc, [key, value]) => ({ ...acc, [key]: value }),
      {}
    );
    let nonTruncatableContent = void 0;
...
var _getTwoBiggestTruncables = (parts) => {
  ...
  for (const part of parts.flat()) {
    if (part.truncatable) {
      if (part.tokens <= (_nullishCoalesce(((_a = part.attributes) == null ? void 0 : _a.minTokens), () => ( 0)))) {
        continue;
      }
      const flex = _nullishCoalesce(((_b = part.attributes) == null ? void 0 : _b.flex), () => ( DEFAULT_TRUNCATE_OPTIONS.flex));
